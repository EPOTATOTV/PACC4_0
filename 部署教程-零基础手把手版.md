...
**第 4 步**：右键 `.env` → 打开方式 → **记事本**，找到下面几行，把 `=` 后面的内容改成你自己的密码：

```
MYSQL_ROOT_PASSWORD=pacc-root-change-me
MYSQL_PASSWORD=pacc-secret-change-me
PACC_ADMIN_API_KEY=pacc-admin-secret-key
PACC_JWT_SECRET=PACC-v4-runtime-secret-key-change-me-in-production-0123456789
```
...
**第 4 步**：用键盘**方向键**把光标移到要改的密码行，删掉旧内容、输入新密码（需要改的是下面 4 行）：

```
MYSQL_ROOT_PASSWORD=改成你的数据库超级密码
MYSQL_PASSWORD=改成你的数据库普通密码
PACC_ADMIN_API_KEY=改成你的后台登录密钥
PACC_JWT_SECRET=改成你的令牌密钥（越长越好，>=32字符）
```
...
系统启动时（后端程序启动那一刻）会用 Hibernate 自动执行"建表"操作（专业叫 `ddl-auto: update`），**自动创建 17 张表**。
...
**看到这些表就对了**：列出 `t_account`、`t_device`、`t_login_event` 等 17 张表。

## 5.3 完整建表 DDL（参考 · 每条都带字段注释）

> ⚠️ 说明：以下 DDL 是"参考版"，方便你理解每张表和每个字段。系统实际自动生成的建表语句与下表**功能一致**，仅细微类型可能不同（如布尔字段系统生成 `bit(1)`，与下面的 `tinyint(1)` 等价）。**不要手动执行这些语句**（除非你在用外部数据库），否则可能与自动建表冲突。
...
*配套快速版见 [部署说明-零基础版.md](部署说明-零基础版.md)。*
