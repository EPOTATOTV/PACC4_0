...
CREATE TABLE t_detection_event (
    ...
    edition            ENUM('BEDROCK','JAVA'),
    client_risk_score  INT,
    process_name       VARCHAR(255),
    memory_region      VARCHAR(255),
    signature_hit      VARCHAR(255),
    detail_json        VARCHAR(255),
...
CREATE TABLE t_signature (
    ...
    pattern          VARCHAR(255),
    risk_level       INT,
    edition          ENUM('BEDROCK','JAVA','GENERIC'),
...
CREATE TABLE t_suspicion_flag (
    ...
    pteid            VARCHAR(255),
    kind             ENUM('SHARED_ACCOUNT_MULTI_DEVICE','SHARED_ACCOUNT_MULTI_IP','DEVICE_FLAPPING','MEDIUM_CONFIDENCE','HARDWARE_CHEAT','TAMPERED_INTEGRITY','UNKNOWN'),
    detail           VARCHAR(255),
    weight           INT,
    evidence_summary VARCHAR(2048),
    prev_chain_hash  VARCHAR(255),
    chain_hash       VARCHAR(255),
    status           ENUM('OPEN','REVIEWED','ESB'),
...
CREATE TABLE t_tournament_stage (
    ...
    order_no       INT,
    title          VARCHAR(255),
    kind           ENUM('QUALIFIER','GROUP','KNOCKOUT','FINAL','CUSTOM'),
