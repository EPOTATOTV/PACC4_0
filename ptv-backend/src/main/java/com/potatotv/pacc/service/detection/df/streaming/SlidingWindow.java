 ...
 * 各做常数次加减（运行和 / 平方和 / 间隔和），因此窗口维护同样是 O(1)/事件，不随窗口或历史长度增长。</p>
 *
 * <p>窗口提供的 {@link #ratePerSecond()} 与 {@link #burstScore()} 是快速规则层的输入：
 * 到达速率突变（{@code burst}）往往先于单点信号超限暴露连点器 / 脚本的节律特征。</p>
 */
public final class SlidingWindow {
    ...
    public void add(double signal, long timestampNanos) {
        if (size == signals.length) {
            ...
            sumIntervalSq -= (double) intervals[oldest] * intervals[oldest];
            head = (head + 1) & mask;
            size--;
        }
        ...
        sumIntervalSq += (double) interval * interval;
     ...
     *
     * <p>机器/脚本的到达间隔高度规律（偏离小反而异常），而人类输入间隔方差大；此处以
     * 「最近间隔显著短于窗口均值」作为突发信号，值为 0 表示无突变。</p>
     */
