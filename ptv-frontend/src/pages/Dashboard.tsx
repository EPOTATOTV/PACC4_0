...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      <Row gutter={[12, 12]} className="pacc-stagger" style={{ marginBottom: 16 }}>
        <Col xs={12} sm={12} md={6}><MetricCard label="在线玩家 (PTEID)" value={summary?.online_pteid ?? '-'} accent="var(--kpi-green)" /></Col>
        <Col xs={12} sm={12} md={6}><MetricCard label="今日检测事件" value={summary?.detections_today ?? '-'} accent="var(--kpi-blue)" /></Col>
        <Col xs={12} sm={12} md={6}><MetricCard label="红屏事件(7天)" value={summary?.redscreen_count ?? '-'} accent="var(--kpi-red)" /></Col>
        <Col xs={12} sm={12} md={6}><MetricCard label="待查端" value={summary?.pending_inspect ?? '-'} accent="var(--kpi-amber)" /></Col>
        <Col xs={12} sm={12} md={6}><MetricCard label="进行中查端" value={summary?.active_inspect ?? '-'} accent="var(--kpi-muted)" /></Col>
        <Col xs={12} sm={12} md={6}><MetricCard label="永久作弊记录" value={summary?.total_cheat_records ?? '-'} accent="var(--kpi-red)" /></Col>
        <Col xs={12} sm={12} md={6}><MetricCard label="注册账号" value={summary?.total_accounts ?? '-'} accent="var(--kpi-blue)" /></Col>
        <Col xs={12} sm={12} md={6}><MetricCard label="基岩 / Java 玩家" value={`${summary?.bedrock_players ?? '-'} / ${summary?.java_players ?? '-'}`} accent="var(--kpi-green)" /></Col>
      </Row>
